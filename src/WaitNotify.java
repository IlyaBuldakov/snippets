/*
 Условия задачи: написать программу с двумя потоками, один из которых исполняет
 роль грузчика (добавляет товары на склад), а второй - потребителя (забирает товары со склада).
 Потоки должны быть синхронизированы и если на складе нет предметов, то счётчик не идет в минус,
 а поток засыпает и в дело вступает грузчик. И только после пополнения склада будет произведена
 операция потребления предметов со склада.
 */
